...
class RedisLib:
    ...
    def _prefix(self, val: str | None = None) -> str:
        if val is None:
            val = self.prefix
        return str(val)

    def lua_script_storage(self, script):
        ...
        if LUA_SCRIPT_STORAGE[client_id].get(script) is None:
            LUA_SCRIPT_STORAGE[client_id][script] = self.redis.register_script(script)
    ...
    def purge_by_prefix(self, prefix: str | None):
        ...
        """
        script = self.lua_script_storage(PURGE_BY_PREFIX)
        script(args=[self._prefix(prefix)])

    def incr_counter(self, name: str, limit: int = 0, expire: int = 0):
        ...
        """
        script = self.lua_script_storage(INCR_COUNTER)
        name = self.prefix + "counter_" + self.secret_hash(name)
        c = script(args=[limit, expire], keys=[name])
        return c

    def drop_counter(self, name: str):
    ...
    def incr_sliding_window(self, name: str, duration: int):
        ...
        """
        script = self.lua_script_storage(INCR_SLIDING_WINDOW)
        name = self.prefix + "counter_" + self.secret_hash(name)
